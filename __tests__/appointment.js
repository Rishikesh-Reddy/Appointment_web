...
describe('Testing Functionalities of Appointment', () => { 
    ...
    test("Testing adding the appointment", async () => {
        ...
        res = await agent.post("/appointment").send({
            ...
            start_time: new Date(new Date().setMinutes(new Date().getMinutes() + 30)).toTimeString(),
            end_time: new Date(new Date().setMinutes(new Date().getMinutes() + 90)).toTimeString(),
        })
    ...
    test("Testing adding appointment with invalid date", async () => {
        ...
        res = await agent.post("/appointment").send({
            ...
            title: "Testing Appointment",
            description: "Testing Description",
            appointmentDate: new Date(new Date().setDate(new Date().getDate() - 1)).toLocaleDateString(),
            start_time: new Date(new Date().setMinutes(new Date().getMinutes() + 30)).toTimeString(),
            end_time: new Date(new Date().setMinutes(new Date().getMinutes() + 90)).toTimeString(),
        })
        expect(res.statusCode).toEqual(302);
